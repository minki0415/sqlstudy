...
- create는 트랜잭션과  무관
- select/insert/update/delete 문장의 필수 항목
	: 작업 후에 실제 DB에 저장시에는 반드시 commit; 명령어 필수
	: DB 자체에는 임시 저장소와 영구 저장소로 분리 
		insert, update, delete 작업은 commit 명령어를 전까지는 임시 저장소에 저장 
		-> commit 실행시 영구 저장소로 이관
		-> rollback 실행 시 임시 저장소에서 휘발

	- 트랜잭션
		여러작업을 마치 하나의 작업으로 간주 (ex : 계좌이체의 개별 작업들이 다 성공시에만 실제 계좌이체 성공)
		- commit /rollback
			commit : 영구저장
			rollback : 복원
	
	- application으로 개발 시 DB에 insert 이후 commit 안하고 데이터 검색시 검색 불가 문제 발생 

	- 참고 : dbeaver는 기본이 auto commit
*/
...
-- ? (11, 유재석, 10) -> emp01 에 추가 / (22, 백종원 ,20) -> emp02에 추가
...
-- 서브쿼리 사용
-- loc에 무의미한 데이터로 갱신 시도시 0row update 메시지 받음
-- 유의미한 데이터 갱신시 3row update 메시지 받음 
drop table emp01;
create table emp01 as select * from emp;
select * from emp01;
...
-- *** delete ***
-- truncate 명령어는 tx처리가 불가능한 데이터 삭제 명령어
-- delete 는 tx 처리 필수 명령어
...
using produce01 p1
on (t.판매번호 = p1.판매번호) -- 비교하는 부분 
when matched then -- match 되었을 때
