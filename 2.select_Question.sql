...
*/
-- 해당 사원 존재하지 않음
select hiredate, ename, deptno, sal from emp where hiredate = '83/01/12';
...
*/
select sal, ename, sal, deptno from emp where sal > 800;
...
*/
select hiredate, ename, sal, deptno from emp where hiredate between '81/04/02' and '81/12/09';
...
/**이름이 S로 시작하고 4번째 글자가 T인 사람이 정보를 출력하라. 단 이름은 전체 5자리이다.
*/
...
부서가 30번 부서이고 급여가 1500이상인 사람의 이름, 부서번호, 월급(sal)을 출력하라.
*/
...
*/
select empno, ename from emp where deptno = 30 order by empno asc;
